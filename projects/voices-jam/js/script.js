...
function setRejection() {
    say('You do not have the proper clearance to check these files. Please obtain proper authorization.');
}

function setMeeting(data) {
    if (data[1] === "are you" || data[1] === "is sister") {
        say('I am the control system for the Nostromo. I assist with navigation and data collection. You, Commander, have acces to my numerous files.');
    }
    else if (data[1] === "is the cat" || data[1] === "is jonesy") { //reference to cat from Alien(1979)
        say('A cat residing on the Nostromo. According to the crew, he is the ships mascot.');
    }
    else if (data[1] === "is rebecca jorden" || data[1] === "is newt") { //reference to character from Aliens(1986)
        say('A young stowaway from a nearby colony. Status of family unknown.');
    }
        ...
        say('A member of the Marine Corps. His team nicknamed him The Blood Dragon.');
    }
...
function setInfo(data) {
    if (data[1] === "is your directive" || data[1] === "is special order nine three seven") { //reference to MU/TH/UR 6000 scene from Alien (1979)
        say('Nostromo rerouted to new co-ordinates. Investigate life form, gather specimen. Priority one, insure return of organism for analysis. All other considerations secondary. Crew expendable.');
    }
    else if (data[1] === "is a xeno morph") {
        say('The sole living organism of dwarf planet RS-79. Near impossible to subdue. Threat level high. Approach with combustible materials. Flamethrower recommended.')
    }
    else if (data[1] === "is a face hugger") {
        say('the first state of a xenomorph after it hatches. When it frees itself, it will attach itself to the face of the closest living organism and lay eggs in their stomach.')
    }
}
...
function setReason(data) {
    ...
    if (data[1] === "are we doing this" || data[1] === "must we do this" || data[1] === "do we have to stop") {
        say('The Wainright Atari corporation has made it their mission and priority to collect and study any new species a ship may come across. No frontier left unchecked.');
    }
}
